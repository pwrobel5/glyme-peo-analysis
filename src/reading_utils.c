...
void read_cation_data(FILE *input_file, vector_t **cation_positions, system_compound_t compound, int *shift)
{
...
void read_ligand_molecule_data(FILE *input_file, vector_t **tracked_positions, system_compound_t compound)
{
...
void read_anion_data(FILE *input_file, vector_t **tracked_positions, system_compound_t anion, int *shift)
{
...
void omit_other_data(FILE *input_file, system_compound_t compound)
{
...
FILE **open_cation_files(system_info_t *system_info)
{
    ...
    for (int i = 0; i < system_info->compounds_number; i++) {
        system_compound_t compound = system_info->compounds[i];
...
{
    current_index += 1;
    system_compound_t current_compound = compounds[current_index];
...
void initialize_entry_data(entry_data_t entry_data, system_info_t *system_info, entry_type_t entry_type)
{
    int ***last_coordination = entry_data.last_coordination;
    int ***current_coordination = entry_data.current_coordination;    
    ligand_coord_info_t ***coordination_info = entry_data.coordination_info;
    ...
    for (int i = 0; i < types_number; i++) {
        ...
        system_compound_t current_compound = system_info->compounds[current_entry_index];
        ...
        for (int j = 0; j < current_compound.quantity; j++) {
            ...
            coordination_info[current_molecule_index] = malloc(current_compound.tracked_atoms_number * sizeof(ligand_coord_info_t*));
            ...
            for (int k = 0; k < current_compound.tracked_atoms_number; k++) {
                ...
                coordination_info[current_molecule_index][k] = malloc(MAX_COORDINATED_CATIONS * sizeof(ligand_coord_info_t));
...
void free_coordination_arrays(entry_data_t entry_data, system_info_t *system_info, entry_type_t entry_type)
{
    int ***last_coordination = entry_data.last_coordination;
    int ***current_coordination = entry_data.current_coordination;    
    ligand_coord_info_t ***coordination_info = entry_data.coordination_info;
...
void open_solvent_data_files(FILE ***files, system_info_t *system_info)
{
...
void close_solvent_data_files(FILE ***files, system_info_t *system_info)
{
    int compound_index = -1;
    for (int i = 0; i < system_info->solvent_types_number; i++) {
        compound_index = get_next_entry_index(compound_index, system_info->compounds, system_info->compounds_number, solvent);
...
void read_box_sizes(program_configuration_t *program_configuration, FILE *box_sizes_file)
{
    char buffer[MAX_LINE_LENGTH];
...
void read_data(program_configuration_t *program_configuration, system_info_t *system_info)
{
    ...
    vector_t **cation_tracked_positions = malloc(system_info->cations_number * sizeof(vector_t*));
    vector_t ***solvent_tracked_positions = malloc(system_info->solvent_types_number * sizeof(vector_t**));
    vector_t **anion_tracked_positions = malloc(system_info->anions_number * sizeof(vector_t*));
    ...
    index_combinations_t **solvent_index_combinations = NULL;
    venn_set_t **venn_set_cations = NULL;
    venn_set_t *venn_set_solvent = NULL;
    if (program_configuration->calculate_venn_diagrams == 1) {
        solvent_index_combinations = malloc(system_info->solvent_types_number * sizeof(index_combinations_t*));
        venn_set_cations = malloc(system_info->solvent_types_number * sizeof(venn_set_t*));
        venn_set_solvent = malloc(system_info->solvent_types_number * sizeof(venn_set_t));
    ...
    for (int i = 0; i < system_info->compounds_number; i++) {
        system_compound_t current_compound = system_info->compounds[i];

        switch (current_compound.entry_type) {
            case cation:
                for (int j = 0; j < current_compound.quantity; j++) {
                    int current_index = j + cation_index;
                    cation_tracked_positions[current_index] = malloc(current_compound.tracked_atoms_number * sizeof(vector_t));
            ...
            case solvent:
                solvent_tracked_positions[solvent_type_index] = malloc(current_compound.quantity * sizeof(vector_t*));
                ...
                for (int j = 0; j < current_compound.quantity; j++) {
                    solvent_tracked_positions[solvent_type_index][j] = malloc(current_compound.tracked_atoms_number * sizeof(vector_t));
                ...
                if (program_configuration->calculate_venn_diagrams == 1) {
                    ...
                    venn_set_cations[solvent_type_index] = malloc(current_compound.quantity * sizeof(venn_set_t));
            ...
            case anion:
                for (int j = 0; j < current_compound.quantity; j++) {
                    ...
                    anion_tracked_positions[current_index] = malloc(current_compound.tracked_atoms_number * sizeof(vector_t));
    ...
    ligand_coord_info_t ***solvent_coordination_info = malloc(system_info->solvent_molecules_number * sizeof(ligand_coord_info_t**));
    
    entry_data_t solvent_data;
    ...
    ligand_coord_info_t ***anion_coordination_info = malloc(system_info->anions_number * sizeof(ligand_coord_info_t**));

    entry_data_t anion_data;
    ...
    venn_diagram_t ***venn_diagrams_cations = malloc(system_info->solvent_types_number * sizeof(venn_diagram_t**));
    venn_diagram_t ***venn_diagrams_solvent = malloc(system_info->solvent_types_number * sizeof(venn_diagram_t**));
    ...
    for (int i = 0; i < system_info->solvent_types_number; i++) {
        ...
        system_compound_t current_compound = system_info->compounds[current_index];
    ...
    while (fgets(buffer, MAX_LINE_LENGTH, input_file) != NULL) {
        ...
        if (program_configuration->calculate_venn_diagrams == 1) {
            ...
            for (int i = 0; i < system_info->solvent_types_number; i++) {
                ...
                venn_diagrams_cations[i] = realloc(venn_diagrams_cations[i], (step_number + 1) * sizeof(venn_diagram_t*));
                venn_diagrams_solvent[i] = realloc(venn_diagrams_solvent[i], (step_number + 1) * sizeof(venn_diagram_t*));
        ...
        coordination_input_t coordination_input;
        ...
        for (int i = 0; i < system_info->cation_types_number; i++) {
            ...
            system_compound_t current_cation = system_info->compounds[compound_index];

            for (int j = 0; j < current_cation.quantity; j++) {
                int index = j + cation_shift;
                cation_coord_info_t coord_info = get_coordination_info(index, current_cation.tracked_atoms_number, cation_tracked_positions[index], &coordination_input);
        ...
        for (int i = 0; i < system_info->solvent_types_number; i++) {
            compound_index = get_next_entry_index(compound_index, system_info->compounds, system_info->compounds_number, solvent);
            system_compound_t current_solvent = system_info->compounds[compound_index];
            ...
            if (program_configuration->calculate_venn_diagrams == 1) {
                ...
                venn_diagram_t *venn_diagram = determine_venn_diagram(&(venn_set_solvent[i]), current_solvent.tracked_atoms_number);
            ...
            for (int j = 0; j < current_solvent.quantity; j++) {
                ...
                if (program_configuration->calculate_venn_diagrams == 1) {
                    ...
                    venn_diagram_t *venn_diagram = determine_venn_diagram(&(venn_set_cations[i][j]), current_solvent.tracked_atoms_number);
        ...
        for (int i = 0; i < system_info->anion_types_number; i++) {
            ...
            system_compound_t current_anion = system_info->compounds[compound_index];
