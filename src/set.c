...
void save_index_set(int *stack, int position, index_set_t *index_set, int index_set_position)
{
...
void determine_powerset(int tracked_atoms_number, index_combinations_t *combination_set)
{
...
index_combinations_t *get_index_combinations(int tracked_atoms_number)
{
    ...
    index_combinations_t *result = malloc(sizeof(index_combinations_t));
    if (result == NULL) raise_error("Error with memory allocation for index combinations");

    result->combinations_number = powerset_size;
    result->combinations = malloc(powerset_size * sizeof(index_set_t));
...
void add_element_to_result_set(index_set_t *result, int index, int element)
{
    result->size += 1;
...
{
    index_set_t *result = malloc(sizeof(index_set_t));
...
void determine_venn_sets(venn_set_t *venn_set, int **current_coordination, int tracked_atoms_number, index_combinations_t *index_combinations, int current_coordination_size)
{
    ...
    venn_set->entries = malloc(venn_set->entries_number * sizeof(venn_entry_t));
    ...
    for (int i = 0; i < index_combinations->combinations_number; i++) {
        ...
        index_set_t current_combination = index_combinations->combinations[i];
        index_set_t *first_set = venn_set->entries[current_combination.indices[0]].set;
        index_set_t *second_set = venn_set->entries[current_combination.indices[1]].set; // combinations contain at least 2 elements
        index_set_t *tmp = NULL;
...
void print_venn_set(venn_set_t *venn_set)
{
    for (int i = 0; i < venn_set->entries_number; i++) {        
        venn_entry_t current_entry = venn_set->entries[i];
        ...
        if (current_entry.set->size > 0) {
            ...
            index_set_t *set = current_entry.set;
            printf("Set indices: ");
...
{
    venn_diagram_t *result = malloc(sizeof(venn_diagram_t));
    ...
    result->entries = malloc(result->entries_number * sizeof(venn_diagram_entry_t));
...
venn_diagram_t *determine_venn_diagram(venn_set_t *venn_set, int tracked_atoms_number)
{
    ...
    for (int i = 0; i < venn_set->entries_number; i++) {
        ...
        index_set_t *differences[tracked_atoms_number - current_entry.entry_id_size + 1]; // + 1 to have non zero size array and keep original set at 0 position
        ...
        while (tracked_atom_index < tracked_atoms_number) {      
            if (entry_id_index >= current_entry.entry_id_size || (entry_id_index < current_entry.entry_id_size && tracked_atom_index < current_entry.entry_id[entry_id_index])) {
                index_set_t *set_to_remove = venn_set->entries[tracked_atom_index].set;
                index_set_t *difference = set_difference(differences[differences_index - 1], set_to_remove);
...
void print_venn_diagram(venn_diagram_t *venn_diagram)
{
    for (int i = 0; i < venn_diagram->entries_number; i++) {        
        venn_diagram_entry_t current_entry = venn_diagram->entries[i];
