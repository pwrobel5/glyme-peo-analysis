...
char *entry_type_to_str(entry_type_t entry_type)
{
...
void parse_system_info_line(system_compound_t *compound, char *buffer)
{
    char *tmp;
...
void count_molecules(system_info_t *system_info)
{
    ...
    for (int i = 0; i < system_info->compounds_number; i++) {
        ...
        entry_type_t entry_type = compound.entry_type;
...
{
    system_info_t *result = malloc(sizeof(system_info_t));
    ...
    result->compounds = malloc(result->compounds_number * sizeof(system_compound_t));
